...
class Article extends Post {
	...
	public function get_featured_video_embed( $media_type, $echo = true ) {
		...
		if ( defined( 'AMP__VERSION' ) && is_amp_endpoint() ) { // legacy
			...
			$markup     = sprintf( $template, 's2nIframe-' . esc_attr( $this->send_to_news_embeds[ $media_type ] ) . '-' . esc_attr( $this->post->ID ), esc_url( $iframe_url ), esc_attr( $styles ), esc_attr( $this->send_to_news_embeds[ $media_type ] ) );
		} else {
